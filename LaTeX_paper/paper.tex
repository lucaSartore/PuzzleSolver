...
\begin{itemize}
  ...
  To do so the program computes the convex hull of the piece,
  then calculates the difference between the convex hull and the original image,
  and obtains an image with the “filler areas”.
  ...
  they are generated by a hole, obtaining an image without holes.

  \item \textbf{Remove the knobs from he piece}\newline
  To do so the image without holes gets eroded and then dilated.
  ...
  but not in the image that has been eroded and dilated. This generates a set of Blobs,
  ...
  close to a Blob generated by a knob, obtaining an image without knobs and holes.
  ...
  side,it just needs to rotate and crop the piece, and so obtain a single side.
\end{itemize}
...
\begin{figure}
  ...
  \begin{subfigure}{0.3\textwidth}
    ...
    \label{fig:s_s_expansion}
  \end{subfigure}
  \vspace{1cm}
  \begin{subfigure}{0.3\textwidth}
    \centering
    \includegraphics[width=\linewidth]{pictures/remove_knobs_knob_pixels.png}
    \caption{Difference:~\ref{fig:s_s_no_holes} \textminus~\ref{fig:s_s_expansion}.}
