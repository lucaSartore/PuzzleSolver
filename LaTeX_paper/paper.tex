...
In fact, the code is not open source, and information regarding the time needed
to solve the puzzle is absent. The article only showcases a single puzzle solved,
...
After reading some of the previous literature on this problem, some conclusions could be drawn:
...
The second objective is to use the research that will be conducted for this paper to create a labeling tool.\newline
...
input data, that are obtained from real-world measure.\newline
...
To fulfill the objective of this paper some data to work with are needed.
This section will cover the input data used as well as the Splitter~\ref{document:splitter}
...
In this section the solver that has been proposed by this paper will be tested.
This test will serve two purposes:
Understanding the performance and limitations of the proposed algorithm
Been able to test the same algorithm with both digital, and real world jigsaw puzzles, to understand the differences between the two.

\subsection{Setup}
The test sample consist in 5 different puzzle:
\begin{itemize}
...
\subsection{Accuracy}
It is important to note that the project does not have a way to evaluate the accuracy
of the results automatically. But the accuracy for the two real world puzzles
has been asserted manually. And both of them are 100\% correct.

\subsection{Analysis}

This paper has claimed from the start that real world puzzles were harder than digital ones.
Some intuitive explanations were given to justify this claim,
but a formal proof has never been given.
This test finally proves the claim, by showing that the same algorithm can have vastly
different time performances depending on the accuracy of the input data.

It is fair to say that the performance of the algorithm are not amazing,
but they are good enough to achieve the objective of this paper \ref{document:objective}.
...
When the program solves a puzzle it also exports some files that could be used in the future to train
a machine-learning based comparator.
This section is here to explain the data available and the format used.

One piece is uniquely identified by his id piece\_id.
The id is an integer that goes from 0 to Number of pieces - 1.
...
for each one of the pieces of the puzzle. The pieces will be named \{piece\_id\}.jpeg.
...
\section{Conclusions}
In conclusion, this paper's objective can be considered achieved.
It has been documented why real-world puzzles are harder than digital puzzles.
It has also been shown that the same algorithm can have drastically worse
performance when working with real-world puzzles compared to digital ones.

A highly effective labeling tool has also been developed.
It could be used in the future to create a machine learning-based comparator that could significantly
improve the time needed to solve this problem by providing more accurate data.

Even the solver that has been created can be considered a success.
His time complexity is horrible if compared with current state-of-the-art digital puzzle solvers.
But if compared with real world one the algorithm is probably one of the best, if not the best, available online.
